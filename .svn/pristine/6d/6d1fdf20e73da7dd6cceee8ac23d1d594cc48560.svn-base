...
class sfWidgetFormInputFileEditable extends sfWidgetFormInputFile
{
  ...
  public function render($name, $value = null, $attributes = array(), $errors = array())
  {
    ...
    return strtr($this->getOption('template'), array('%input%' => $input, '%delete%' => $delete, '%delete_label%' => $deleteLabel, '%file%' => $this->getFileAsTag($attributes), '%filepath%'=> $this->getFileAsPath($attributes)));
  }
  ...
  protected function getFileAsTag($attributes)
  {
    if ($this->getOption('is_image'))
    {
		...
		{
	      return false !== $this->getOption('file_src') ? $this->renderTag('img', array_merge(array('src' => $this->getOption('file_src'),'height'=>$this->getOption('height')), $attributes)) : '';
		}else{
		  return false !== $this->getOption('file_src') ? $this->renderTag('img', array_merge(array('src' => $this->getOption('file_src'),'width'=>$this->getOption('width')), $attributes)) : '';
		}
    }
    else
    {
      return sprintf('<a href="%1$s" target="_blank">Download file</a>', $this->getOption('file_src'));
    }
  }
  protected function getFileAsPath($attributes)
  {
      return sprintf('<a href="%1$s" target="_blank" style="color:#80BFFF">link image</a>', $this->getOption('file_src'));
  }
}
