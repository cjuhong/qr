...
class sfWidgetFormInput extends sfWidgetForm
{
  ...
  protected function configure($options = array(), $attributes = array())
  {
    ...
    $this->addOption('is_readonly', true);
  }
  ...
  {
if ($this->getOption('is_readonly'))
    {
    return $this->renderTag('input', array_merge(array('type' => $this->getOption('type'), 'name' => $name, 'value' => $value), $attributes));
	}
    else
    {
    return $this->renderTag('input', array_merge(array('type' => $this->getOption('type'), 'name' => $name,'readonly'=>'', 'value' => $value), $attributes));
//      return sprintf('<label>%1$s</label>', $value);
    }
   }
}
