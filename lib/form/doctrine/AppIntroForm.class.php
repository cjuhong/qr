 ...
 */
class AppIntroForm extends BaseAppIntroForm
{
  public function configure()
  {
    ...
    $imageOne =$this->getObject()->getAppLogo();
    $path = sfConfig::get('app_upload_path_app_intro');
    $path_dir = sfConfig::get('sf_upload_dir').'/app_intro/';
/****************************************************************************/
    if($imageOne){
      $this->widgetSchema['app_logo'] = new sfWidgetFormInputFileEditable(array(
        'label' => 'App Logo',
      ...
      $this->widgetSchema['app_logo'] = new sfWidgetFormInputFile(array(
        'label' => 'App Logo',
      ));
    }
    $this->validatorSchema['app_logo'] = new sfValidatorFile(array(
      'max_size'   => $maxSize*1024*1024,
      'required'   => false,
      'path'       => $path_dir,
      'mime_types' => 'web_images',
    ));
    $this->validatorSchema['app_logo_delete'] = new sfValidatorString(array('required'=>false));
/*********************************************************************************/
...
/************************************************************************/
      $this->widgetSchema['app_logo']->setAttribute('disabled', 'disabled');
      $this->widgetSchema['app_capture']->setAttribute('readonly', 'readonly');
      $this->widgetSchema['company_name']->setAttribute('readonly', 'readonly');
      $this->widgetSchema['app_id'] =new sfWidgetFormInputText();
      $this->widgetSchema['app_id']->setAttribute('readonly', 'readonly');
      $this->widgetSchema['link']->setAttribute('readonly', 'readonly');
    ...
    foreach ($languagesE as $language) 
      {
        if($language == 'en'&& !$user)
          {
            $this->widgetSchema['en']['app_name']->setAttribute('readonly', 'readonly');
            $this->widgetSchema['en']['company_logo']->setAttribute('readonly', 'readonly');
          ...
          $this->widgetSchema[$language]['company_intro'] = new sfWidgetFormTextareaTinyMce(array('label' => 'Company intro'), array('rows' => '25', 'cols' =>'100'));
          $this->widgetSchema[$language]['app_intro'] = new sfWidgetFormTextareaTinyMce(array('label' => 'App intro'), array('rows' => '25', 'cols' =>'100'));
        }
      }
